...
void MixColumns(State& state) {
    for (int c = 0; c < 4; ++c) {
        ...
        state[0][c] = mul(a, 2) ^ mul(b, 3) ^ c1 ^ d;
        state[1][c] = a ^ mul(b, 2) ^ mul(c1, 3) ^ d;
        state[2][c] = a ^ b ^ mul(c1, 2) ^ mul(d, 3);
        state[3][c] = mul(a, 3) ^ b ^ c1 ^ mul(d, 2);
    }
}
...
void InvMixColumns(State& state) {
    for (int c = 0; c < 4; ++c) {
        ...
        state[0][c] = mul(a, 0x0e) ^ mul(b, 0x0b) ^ mul(c1, 0x0d) ^ mul(d, 0x09);
        state[1][c] = mul(a, 0x09) ^ mul(b, 0x0e) ^ mul(c1, 0x0b) ^ mul(d, 0x0d);
        state[2][c] = mul(a, 0x0d) ^ mul(b, 0x09) ^ mul(c1, 0x0e) ^ mul(d, 0x0b);
        state[3][c] = mul(a, 0x0b) ^ mul(b, 0x0d) ^ mul(c1, 0x09) ^ mul(d, 0x0e);
    }
}

void KeyExpansion(const std::vector<uint8_t>& key, std::vector<std::vector<uint8_t>>& roundKeys) {
    roundKeys.resize(Nr + 1, std::vector<uint8_t>(16));
    for (int i = 0; i < 16; ++i)
    ...
    for (int i = Nk; i < Nb * (Nr + 1); ++i) {
        ...
        for (int j = 0; j < 4; ++j)
            roundKeys[i] = {
                (uint8_t)(roundKeys[i - Nk][0] ^ temp[0]),
                (uint8_t)(roundKeys[i - Nk][1] ^ temp[1]),
                (uint8_t)(roundKeys[i - Nk][2] ^ temp[2]),
                (uint8_t)(roundKeys[i - Nk][3] ^ temp[3]),
            };
    }
}
